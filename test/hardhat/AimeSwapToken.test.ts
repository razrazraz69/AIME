...
describe('AimeSwapToken Test', function () {
    // Constant representing a mock Endpoint ID for testing purposes
    const eidA = 1
    const eidB = 2
    // Declaration of variables to be used in the test suite
    let AimeSwapToken: ContractFactory
    ...
    before(async function () {
        ...
        AimeSwapToken = await ethers.getContractFactory('AimeSwapTokenMock')
    ...
    beforeEach(async function () {
        ...
        // Deploying two instances of AimeSwapToken contract with different identifiers and linking them to the mock LZEndpoint
        myOFTA = await AimeSwapToken.deploy('aOFT', 'aOFT', mockEndpointV2A.address, ownerA.address)
        myOFTB = await AimeSwapToken.deploy('bOFT', 'bOFT', mockEndpointV2B.address, ownerB.address)

        // Setting destination endpoints in the LZEndpoint mock for each AimeSwapToken instance
        ...
        // Setting each AimeSwapToken instance as a peer of the other in the mock LZEndpoint
